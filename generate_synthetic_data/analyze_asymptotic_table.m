load('asymptotic_data.mat')

n_horizons = length(simdata);

best_mle_error = zeros(n_horizons, mc_iterations);
worst_mle_error = zeros(n_horizons, mc_iterations);
best_em_error = zeros(n_horizons, mc_iterations);
worst_em_error = zeros(n_horizons, mc_iterations);

% each sensitivity cell contain 
% 2 matrices n_guesses x 1 x n_iter
% 2 matrices n_guesses x nparams x n_iter
for sensitivity=1:n_horizons
