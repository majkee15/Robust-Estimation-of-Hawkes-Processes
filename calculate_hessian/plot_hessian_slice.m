function [] = plot_hessian_slice(p1,p2,p3,cartProd,res,cond_mle,cond_em)
[inpar,params] = setpar();
...
hold on
plot(x(1,:),cond_mle(closestIndex,:))
plot(x(1,:),cond_em(closestIndex,:))
hold off
legend('MLE','EM')
figure;
[minValue,closestIndex] = min(abs(y(:,p2) - params(p2)));
hold on
plot(y(:,1),cond_mle(:,closestIndex))
plot(y(:,1),cond_em(:,closestIndex))
hold off
legend('MLE','EM')
